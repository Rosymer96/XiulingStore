...
for (let product of products) {
  ...
  $product.dataset.productId = product.id;
...
function pushProduct() {
  let $product = this.closest(".product-pill");
  let id = parseInt($product.dataset.productId);
  let productInCar = products.find((product) => product.id === id);
...
function renderShoppingCar() {
  ...
  for (let selectedProduct of selectedProducts) {
    ...
    $selectedProduct.dataset.productId = selectedProduct.id;
  ...
  $totalPrice.textContent = selectedProducts.reduce(
    (acc, selectedProduct) =>
...
function addProduct() {
  let $product = this.closest(".selectedProduct");
  let id = parseInt($product.dataset.productId);
...
function removeProduct() {
  ...
  if (selectedProduct) {
    if (selectedProduct.quantity > 1) {
      ...
      ); // Filtramos todos los productos y creamos un nuevo array que asignamos a selectedProducts.
    }
  }
...
function startOrder() {
  if (selectedProducts.length === 0) {
    alert(
      "El carrito de compras esta vacío y no se puede proceder con la compra."
    );
  } else {
...
$btnCloseMessageOrder.addEventListener("click", () =>
  $messageOrder.classList.remove("active")
);
